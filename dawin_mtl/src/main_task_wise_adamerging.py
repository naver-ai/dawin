...
from eval import eval_single_dataset, eval_single_dataset_head, eval_single_dataset_dynamic, eval_single_dataset_preprocess_head
...
# {'SUN397':0,'Cars':1,'RESISC45':2,'EuroSAT':3,'SVHN':4,'GTSRB':5,'MNIST':6,'DTD':7}
...
class AdaMerging(torch.nn.Module):
    ...
    def set_pt_lambdas(self):
        ralpha = get_merging_cofficients('tw_adamerging', args.model)
...
if args.eval_only:
    for dataset_name in exam_datasets:
        if args.set_alphas == 'ours':
            startt = time.time()
            metrics = eval_single_dataset_dynamic(adamerging_mtl_model,dataset_name,args,dmm)
        else:
    ...
    for epoch in range(epochs):
        ...
        for dataset_name in exam_datasets:
            ...
            for i, data in enumerate(tqdm.tqdm(dataloader)):
                ...
                outputs = adamerging_mtl_model(x, None, dataset_name)
